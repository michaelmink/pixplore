...
class WorkerService(service_pb2_grpc.WorkerServiceServicer):
    async def ProcessTask(self, request, context):
        ...
        thumbnail_path = os.path.join(thumbnail_dir, os.path.basename(img_path).rsplit(".", 1)[0] + "_thumb.jpg")
        ...
        try:
            with Image.open(img_path) as img:
                ...
                return service_pb2.TaskResponse(status="COMPLETED", db_record_id=task_id)
