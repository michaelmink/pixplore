...
class FaceDetectorSCRFD:
    ...
    def detect_faces(
        self, img: Image.Image, img_path: Path, debug=True, th=0.5
    ) -> list:
        """
        ...
        for i, bbox in enumerate(bboxes):
            ...
            crop_pil.save(
                os.path.join(
                    self.CONFIG["dataset"]["label_path"],
                    f"{img_path.stem}_face_crop_{i:03d}.jpg",
                )
            )
        ...
        if debug:
            ...
            img.save(
                os.path.join(
                    self.CONFIG["dataset"]["debug_path"], f"{img_path.stem}_debug.jpg"
                )
            )
